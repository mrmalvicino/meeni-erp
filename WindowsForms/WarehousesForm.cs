...
namespace WindowsForms
{
    public partial class WarehousesForm : Form
    {
        ...
        private void applyFilter()
        {
            ...
            if (2 < filter.Length)
                _filteredWarehouses = _warehousesTable.FindAll(reg => ((reg.ActiveStatus && showActive) || (!reg.ActiveStatus && showInactive)) && (reg.FirstName.ToUpper().Contains(filter.ToUpper()) || reg.LastName.ToUpper().Contains(filter.ToUpper()) || reg.BusinessName.ToUpper().Contains(filter.ToUpper()) || reg.BusinessDescription.ToUpper().Contains(filter.ToUpper()) || reg.Email.ToUpper().Contains(filter.ToUpper()) || reg.LegalId.ToString().Contains(filter)));
            else
                _filteredWarehouses = _warehousesTable.FindAll(reg => (reg.ActiveStatus && showActive) || (!reg.ActiveStatus && showInactive));
        ...
        private void dataGridView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            {
                _selectedWarehouse = (Warehouse)dataGridView.CurrentRow.DataBoundItem;
                Functions.loadImage(pictureBox, _selectedWarehouse.ImageUrl);
                loadProfile(_selectedWarehouse.Id, _selectedWarehouse.ToString(), _selectedWarehouse.BusinessDescription, _selectedWarehouse.Phone.ToString(), _selectedWarehouse.Email.ToString(), _selectedWarehouse.Adress.ToString());
            }
        }
        ...
        {
            WarehouseRegisterForm registerForm = new WarehouseRegisterForm(_selectedWarehouse);
            registerForm.ShowDialog();
            refreshTable();
            applyFilter();
        }
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    _warehousesManager.delete(_selectedWarehouse.Id);
                    refreshTable();
                    applyFilter();
                }
            }
